...
from ..models import KabaddiPitch, KabaddiSchedule, KabaddiTable, \
    KabaddiTeam, KabaddiKnockout
...
def generate_schedule() -> None:
    ...
    if KabaddiSchedule.objects.all().exists():
        return

    # Randomly assign teams to groups
    teams = KabaddiTeam.objects.all().order_by("name").values("team_id")
    # shuffle the teams
    random.shuffle(list(teams))
    for i, team in enumerate(teams):
        # Assign the team to a group
        KabaddiTeam.objects.filter(team_id=team["team_id"]).update(
            group=i % number_of_groups + 1)

    teams = KabaddiTeam.objects.all().order_by("group", "name").values(
        "team_id", "group")
        ...
        for team in group_teams:
            ...
            for opponent in group_teams:
                ...
                if team["team_id"] != opponent["team_id"]:
                    ...
                    if not KabaddiSchedule.objects.filter(
                        ...
                        if not KabaddiSchedule.objects.filter(
                            ...
                            KabaddiSchedule.objects.update_or_create(
                                team_id=team["team_id"],
                                opponent_id=opponent["team_id"])

    # Randomly assign pitches and times to fixtures
    pitches = KabaddiPitch.objects.all().order_by("name")
    pitches_count = pitches.count()

    fixtures = KabaddiSchedule.objects.filter(played=False).order_by(
    ...
    for index, fixture in enumerate(fixtures):
        ...
        KabaddiSchedule.objects.filter(
...
def get_kabaddi_schedule() -> dict:
    ...
    if not KabaddiSchedule.objects.all().exists():
        generate_schedule()

    initalise_kabaddi_table()

    schedule = KabaddiSchedule.objects.select_related(
        "pitch").all().order_by(
...
def get_kabaddi_table() -> dict:
    ...
    table = KabaddiTable.objects.all().select_related(
    ...
    groups = KabaddiTeam.objects.all().values("group").distinct().order_by(
        "group")
...
def update_kabaddi_table(team_id: int) -> None:
    ...
    team_results = KabaddiSchedule.objects.filter(
...
def get_unplayed_kabaddi_games() -> list:
    ...
    games = KabaddiSchedule.objects.filter(played=False).order_by(
        "pitch_id__name",
        "time").values(
        "schedule_id",
        "team__name",
    ...
    knockout_games = KabaddiKnockout.objects.filter(played=False).order_by(
        "step_id").values(
        "id",
        "team__name",
        "opponent__name",
...
def generate_quarter_final() -> None:
    ...
    while len(knockout_teams) < 8:
        next_best_team = KabaddiTable.objects.exclude(
    ...
    knockout_teams = KabaddiTable.objects.filter(
        team_id__in=knockout_teams).order_by("-points_per_game",
                                             "-goal_difference",
                                             "-goals_for").values(
        "team_id")
...
def generate_final() -> None:
    ...
    if KabaddiKnockout.objects.filter(
            step_id=5).exists() or KabaddiKnockout.objects.filter(
        step_id=6).exists():
...
def log_kabaddi_score(schedule_id: int, home_score: int,
    ...
    if KabaddiSchedule.objects.filter(played=False).exists():
        ...
        update_kabaddi_table(KabaddiSchedule.objects.get(
