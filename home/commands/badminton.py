...
from ..models import BadmintonPitch, BadmintonSchedule, BadmintonTable, \
    BadmintonTeam, BadmintonKnockout
...
def generate_schedule() -> None:
    """
    Generate the badminton schedule.
        ...
        for team in group_teams:
            ...
            for opponent in group_teams:
                ...
                if team["team_id"] != opponent["team_id"]:
                    ...
                    if not BadmintonSchedule.objects.filter(
                        ...
                        if not BadmintonSchedule.objects.filter(
                            ...
                            BadmintonSchedule.objects.update_or_create(
                                team_id=team["team_id"],
                                opponent_id=opponent["team_id"])

    pitches = BadmintonPitch.objects.all().order_by("name")
    pitches_count = pitches.count()

    fixtures = BadmintonSchedule.objects.filter(played=False).order_by(
    ...
    for index, fixture in enumerate(fixtures):
        ...
        BadmintonSchedule.objects.filter(
...
def get_badminton_schedule() -> dict:
    ...
    if not BadmintonSchedule.objects.all().exists():
        generate_schedule()

    initalise_badminton_table()

    schedule = BadmintonSchedule.objects.select_related(
        "pitch").all().order_by(
...
def get_badminton_table() -> dict:
    ...
    table = BadmintonTable.objects.all().select_related(
    ...
    groups = BadmintonTeam.objects.all().values("group").distinct().order_by(
        "group")
...
def update_badminton_table(team_id: int) -> None:
    ...
    team_results = BadmintonSchedule.objects.filter(
...
def get_unplayed_badminton_games() -> list:
    ...
    games = BadmintonSchedule.objects.filter(played=False).order_by(
        "pitch_id__name",
        "time").values(
        "schedule_id",
        "team__name",
    ...
    knockout_games = BadmintonKnockout.objects.filter(played=False).order_by(
        "step_id").values(
        "id",
        "team__name",
        "opponent__name",
...
def generate_quarter_final() -> None:
    ...
    while len(knockout_teams) < 8:
        next_best_team = BadmintonTable.objects.exclude(
    ...
    knockout_teams = BadmintonTable.objects.filter(
        team_id__in=knockout_teams).order_by("-points_per_game",
                                             "-goal_difference",
                                             "-goals_for").values(
        "team_id")
...
def generate_final() -> None:
    ...
    if BadmintonKnockout.objects.filter(
            step_id=5).exists() or BadmintonKnockout.objects.filter(
        step_id=6).exists():
...
def log_badminton_score(schedule_id: int, home_score: int,
    ...
    if BadmintonSchedule.objects.filter(played=False).exists():
        ...
        update_badminton_table(BadmintonSchedule.objects.get(
